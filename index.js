...
$(document).ready(function() {
    ...
    var newPage = (n) => {
        ...
        for (var i = 1; i <= pageNum; i++) {
            ...
            $a.on('click', function() {
                ...
                $('li.page-item').attr('class', 'page-link').attr('class', 'page-item')
        ...
        $lli.on('click', function() {
            if (now_page > 1) {
                ...
                $('li.page-item').eq(Number(now_page)).addClass('active')

            } else {
                $('li.page-item').eq(Number(now_page - 1)).addClass('disabled')
        ...
        $rli.on('click', function() {
            ...
            $('li.page-item').eq(Number(now_page)).addClass('active')
            if (now_page == pageNum) {
                $('li.page-item').eq(Number(now_page + 2)).addClass('disabled')
            }
        })
    ...
    $('#query').on('click', function() {
        $.get('https://js.kchen.club/B07106018/query', function(response) {
            if (response) {
                // 伺服器有回傳資料
                if (response.result) {
                    $('#product-list').empty();
